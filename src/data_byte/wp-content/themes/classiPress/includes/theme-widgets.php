    ...
    <div class="shadowblock_out">
        ...
        <div class="shadowblock">
            ...
            <div class="recordfromblog">
                ...
                <ul>
                    <?php wp_list_categories('orderby=name&order=asc&hierarchical=1&show_count=1&use_desc_for_title=0&hide_empty=0&depth=1&number=&title_li=&taxonomy=ad_cat'); ?>
                </ul>
    ...
    <div class="recordfromblog">
        ...
        <form action="<?php bloginfo('url'); ?>/" method="get" id="searchform" class="form_search">
            ...
            <input name="s" type="text" id="s" class="editbox_search" <?php if(get_search_query()) { echo 'value="'.trim(strip_tags(esc_attr(get_search_query()))).'"'; } else { ?> value="<?php _e('What are you looking for?','appthemes'); ?>" onfocus="if (this.value == '<?php _e('What are you looking for?','appthemes'); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e('What are you looking for?','appthemes'); ?>';}" <?php } ?> />
                ...
                <?php wp_dropdown_categories('show_option_all='.__('All Categories', 'appthemes').'&hierarchical='.get_option('cp_cat_hierarchy').'&hide_empty='.get_option('cp_cat_hide_empty').'&depth='.get_option('cp_search_depth').'&show_count='.get_option('cp_cat_child_count').'&orderby='.get_option('cp_cat_orderby').'&title_li=&use_desc_for_title=1&taxonomy=ad_cat'); ?>

            <input type="submit" class="btn_orange" value="<?php _e('Search','appthemes')?>" title="Search" id="go" name="sa" />
        </form>
    ...
    <div class="shadowblock_out">
        ...
        <div class="shadowblock">
            ...
            <ul>
                ...
                $sql = $wpdb->prepare("SELECT field_values "
                     . "FROM ". $wpdb->prefix . "cp_ad_fields f "
                     . "WHERE f.field_name = 'cp_city'");
                ...
                if($results) { ?>
                  ...
                  echo __('No cities found.', 'appthemes');
...
class CP_Widget_Ads_Tag_Cloud extends WP_Widget {
	...
	?>
			...
			<?php foreach ( get_object_taxonomies('ad_listing') as $taxonomy ) :
